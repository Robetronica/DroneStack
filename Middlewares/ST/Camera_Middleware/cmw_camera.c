...
void HAL_DCMIPP_MspInit(DCMIPP_HandleTypeDef *hdcmipp)
{
  ...
  periph_clk.ICSelection[RCC_IC17].ClockSelection = RCC_ICCLKSOURCE_PLL1;
  periph_clk.ICSelection[RCC_IC17].ClockDivider = 4;
  periph_clk.ICSelection[RCC_IC18].ClockSelection = RCC_ICCLKSOURCE_PLL1;
  periph_clk.ICSelection[RCC_IC18].ClockDivider = 60;
...
static int32_t CMW_CAMERA_IMX335_Init(CMW_Sensor_Init_t *initSensors_params)
{
  ...
  camera_bsp.imx335_bsp.Init        = I2C1_BusInit;
  camera_bsp.imx335_bsp.DeInit      = I2C1_BusDeInit;
  camera_bsp.imx335_bsp.ReadReg     = I2C1_ReadReg16;
  camera_bsp.imx335_bsp.WriteReg    = I2C1_WriteReg16;
  camera_bsp.imx335_bsp.GetTick     = I2C1_GetTick;
...
static int32_t CMW_CAMERA_SetPipe(DCMIPP_HandleTypeDef *hdcmipp, uint32_t pipe, CMW_DCMIPP_Conf_t *p_conf, uint32_t *pitch)
{
  ...
  if (p_conf->yuv_conversion)
  {
    ...
    if (HAL_DCMIPP_PIPE_SetYUVConversionConfig(hdcmipp, pipe, &color_conf) != HAL_OK)
    {
      return CMW_ERROR_COMPONENT_FAILURE;
    }
    if (HAL_DCMIPP_PIPE_EnableYUVConversion(hdcmipp, pipe) != HAL_OK)
    {
      return CMW_ERROR_COMPONENT_FAILURE;
    }
  }
